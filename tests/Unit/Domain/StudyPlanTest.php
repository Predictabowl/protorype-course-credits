...
class StudyPlanTest extends TestCase{
    ...
    public function test_addExamLik_when_no_exam_present() {
        $block = new ExamBlockDTO(1);
        $option = new ExamOptionDTO("option1", $block, 12, "ssd");
        $taken = new LinkedTakenExam(new TakenExamDTO("taken1", "ssd", 9));
        $takenPk = $taken->getTakenExam()->getPK();
        ...
        $this->assertEquals($option, $this->studyPlan->getExam($option->getPK())
                ->getExamOption());
        $this->assertEquals($taken->getTakenExam(),
                $this->studyPlan->getExam($option->getPK())
                ->getTakenExam($takenPk)->getTakenExam());
    }
    
    public function test_addExamLink_leftover_cfu_values() {
        $block = new ExamBlockDTO(1);
        $option1 = new ExamOptionDTO("option1", $block, 12, "ssd");
        $option2 = new ExamOptionDTO("option2", $block, 12, "ssd");
        $taken1 = new LinkedTakenExam(new TakenExamDTO("taken1", "ssd", 10));
        $taken2 = new LinkedTakenExam(new TakenExamDTO("taken2", "ssd", 6));
        ...
        $this->assertEquals(0, $this->studyPlan->getExam($option1->getPK())
                ->getIntegrationValue());
        $this->assertEquals(8, $this->studyPlan->getExam($option2->getPK())
                ->getIntegrationValue());

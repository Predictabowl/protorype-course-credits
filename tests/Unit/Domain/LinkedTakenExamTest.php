...
class LinkedTakenExamTest extends TestCase
{
    ...
    {
        $sut = new LinkedTakenExam(new TakenExamDTO(1,"name1","ssd1", self::FIXTURE_CFU));
    ...
    public function test_throw_when_actual_cfu_value_exceed_max_value()
    {
        ...
        new LinkedTakenExam(new TakenExamDTO(1,"name","ssd",10),11);
    }
    
    public function test_throw_when_actual_cfu_value_is_not_integer()
    {
        $this->expectException(InvalidArgumentException::class);
        new LinkedTakenExam(new TakenExamDTO(2,"name","ssd",10),"test");
    }

    public function test_throw_when_actual_cfu_value_is_negative()
    {
        $this->expectException(InvalidArgumentException::class);
        new LinkedTakenExam(new TakenExamDTO(3,"name","ssd",10),-1);
    }

    public function test_split()
    {
        //$exam = ;
        $sut = new LinkedTakenExam(new TakenExamDTO(4,"name","ssd", self::FIXTURE_CFU));
        $sut2 = $sut->split(5);
